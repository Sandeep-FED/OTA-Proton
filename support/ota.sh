...
FILENAME=$(find out/target/product/$DEVICE/Project-zephyrus*.zip | cut -d "/" -f 5)
ID=$(sha256sum out/target/product/$DEVICE/Project-zephyrus*.zip | cut -d " " -f 1)
FILEHASH=$(md5sum out/target/product/$DEVICE/Project-zephyrus*.zip | cut -d " " -f 1)
SIZE=$(wc -c out/target/product/$DEVICE/Project-zephyrus*.zip | awk '{print $1}')
URL="https://sourceforge.net/projects/Project-zephyrus/files/$DEVICE/$FILENAME/download"
VERSION="12"
PEXV=$(grep "org.Project-zephyrus.version=" out/target/product/$DEVICE/system/build.prop | cut -d "=" -f 2)
JSON_FMT='{\n"error":false,\n"filename": "%s",\n"datetime": %s,\n"size":%s, \n"url":"%s", \n"filehash":"%s", \n"version": "%s", \n"Project-zephyrus": "%s", \n"id": "%s", \n"tg_username": "%s", \n"device_name":"%s", \n"device":"%s", \n"xda_thread":"%s"\n}'
