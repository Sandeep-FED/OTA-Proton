...
Project-Zephyrus="$(jq -r '.Project-Zephyrus' "${CHANGED_FILE}")"
...
DEVICE="$(jq -r '.device' "${CHANGED_FILE}")"
         ...
         curl -X POST -F photo=@"${PHOTO}" https://api.telegram.org/bot"${TOKEN}"/sendPhoto -F chat_id="${CHAT_ID}" -F parse_mode=Markdown -F "caption=*Project-Zephyrus(ProtonKnockOFF) ${Project-Zephyrus} OFFICIAL Update for ${DEVICE_NAME} is available*
*By* : @${USERNAME}

*Build Version* : ${Project-Zephyrus}
...
*Support* : @ProjectZephyrusCommunity
*Channel* : @ProjectZephyrusUpdates
*Site* : pixelextended.ninja

*Like My Project* [Donate Here](https://www.paypal.com/paypalme/Manikantaraavi)

#Project-Zephyrus(ProtonKnockOFF) #${DEVICE}"
